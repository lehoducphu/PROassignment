...
// Thêm một car 
    ...
    public void ListCarOfBrandName() {
      ...
      int count=0;
        ...
        for (Car car : cList) {
            if(car.getBrand().getBrandName().toUpperCase().contains(input)){
                System.out.println(car);
                count++;
            }
            
        }
        if(count==0) System.out.println("No result");
    ...
    public static String inputStr(String ID, char c, boolean isIncludeDigits) {
        ...
        while (true) {
            if (isIncludeDigits(result) == isIncludeDigits) {
                ...
                System.err.println("You have entered wrong string. Reason: String after 'F' must be digits only");
                System.err.print("Please enter the Frame ID again: ");
                result = inputStr(ID, c);
            } else {
                System.err.println("You have entered wrong string. Reason: String after 'E' must be digits only");
                System.err.print("Please enter the Engine ID again: ");
                result = inputStr(ID, c);
            }
        }
